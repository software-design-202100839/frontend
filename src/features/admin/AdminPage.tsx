  ...
  return (
    <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <UserPlus className="h-4 w-4" /> 교사 등록
          </CardTitle>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <Users className="h-4 w-4" /> 교사 목록 ({teachers.length}명)
          </CardTitle>
        </CardHeader>
        <CardContent>
          <Table>
            ...
            <TableBody>
              {teachers.map((t) => (
                <TableRow key={t.id}>
                  ...
                  <TableCell>
                    <StatusBadge active={t.activated} />
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <UserPlus className="h-4 w-4" /> 학생 등록
            </CardTitle>
          </CardHeader>
          <CardContent>
            ...
            <form onSubmit={handleEnroll} className="space-y-4">
              <div>
                ...
                >
                  ...
                  {students
                    .filter((st) => !st.currentEnrollment)
                    .map((st) => (
                      <option key={st.id} value={st.id}>
                        {st.name} ({st.admissionYear}년 입학)
                      </option>
                    ))}
                </Select>
              </div>
            ...
            <TableBody>
              {students.map((st) => (
                <TableRow key={st.id}>
                  ...
                  <TableCell>
                    <StatusBadge active={st.activated} />
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <UserPlus className="h-4 w-4" /> 학부모 등록
            </CardTitle>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleRegister} className="space-y-4">
            ...
            <TableBody>
              {parents.map((p) => (
                <TableRow key={p.id}>
                  ...
                  <TableCell>
                    {p.children.map((c) => `${c.studentName}(${c.relationship})`).join(', ') || '-'}
                  </TableCell>
                  <TableCell>
                    <StatusBadge active={p.activated} />
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <School className="h-4 w-4" /> 반 생성 ({year}학년도)
            </CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <GraduationCap className="h-4 w-4" /> 담임 배정
            </CardTitle>
          </CardHeader>
          <CardContent>
  ...
  return (
    <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <BookOpen className="h-4 w-4" /> 교사 과목 배정 ({year}학년도)
          </CardTitle>
        </CardHeader>
        <CardContent>
          ...
          <Table>
            ...
            <TableBody>
              ...
              {assignments
                .sort(
                  (a, b) =>
                    a.classInfo.grade - b.classInfo.grade ||
                    a.classInfo.classNum - b.classInfo.classNum,
                )
                .map((a) => (
                  <TableRow key={a.id}>
                    <TableCell>
                      {a.teacher.name}
                      {a.teacher.department ? ` (${a.teacher.department})` : ''}
                    </TableCell>
                    <TableCell>
                      {a.classInfo.grade}학년 {a.classInfo.classNum}반
                    </TableCell>
                    <TableCell>{a.subject.name}</TableCell>
                    <TableCell>
                      <Button onClick={() => handleDelete(a.id)} variant="destructive" size="sm">
...
function StatusBadge({ active }: { active: boolean }) {
  return <Badge variant={active ? 'success' : 'destructive'}>{active ? '활성' : '미활성'}</Badge>;
}
